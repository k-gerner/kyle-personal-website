...
class GomokuStrategy(GomokuPlayer):
	...
	def check_if_move_caused_game_over(
		...
		for row in board:
			for spot in row:
				if spot == BoardSpace.EMPTY:
					# found an BoardSpace.EMPTY spot
					return None, False
		return None, True
	...
	def get_valid_moves(self, board: List[List[BoardSpace]]) -> List[Tuple[int, int]]:
		...
		empties_found_so_far = set()  # used to make sure we don't double-count BoardSpace.EMPTY spots
		...
		def locate_empty_near_spots(filled_coord: Tuple[int, int], dist_from_piece: int):
			...
			for i in range(-dist_from_piece, dist_from_piece + 1):  # e.g. -1, 0, 1
				for j in range(-dist_from_piece, dist_from_piece + 1):  # e.g. -1, 0, 1
					...
					if neighbor_spot == BoardSpace.EMPTY and neighbor_coord_as_tuple not in empties_found_so_far:
						# if there is an BoardSpace.EMPTY spot here and we haven't validated it yet
				...
				if forward_piece_color == backward_piece_color:
					...
					if forward_distance_reached + 1 + num_backward_empties_before_piece < 5 and backward_distance_reached + 1 + num_forward_empties_before_piece < 5:
						...
						if opponent_of(forward_piece_color) == backward_piece_color and forward_piece_color is not None and backward_piece_color is not None:
							...
							# one of the directions is all BoardSpace.EMPTY spaces, and the other contains at least one player piece
	...
	def score_sections(
		...
		if color_of_evaluator == BoardSpace.BLACK:
...
def opponent_of(color: PlayerBoardSpace) -> PlayerBoardSpace:
	...
	return BoardSpace.WHITE.value if color == BoardSpace.BLACK else BoardSpace.BLACK.value
